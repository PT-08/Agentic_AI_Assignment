...
class SolarROIAnalysisAgent:
    ...
    def process(self, state: HouseholdProfileState) -> Command:
        ...
        try:
            ...
            base_kwh = float(energy_metrics.get("adjusted_daily_energy_kWh", energy_metrics.get("daily_energy_consumption_kWh", 0.0)))
            ...
            # Use existing solar estimate if present, otherwise estimate from capacity
            ...
            if solar_capacity > 0:
                assumptions.append(f"Using existing solar capacity = {solar_capacity} kWp -> est {round(solar_gen,3)} kWh/day.")
